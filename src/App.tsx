      ...
      {labelInfo && (
        <div className="country-label">
          ...
          <div className="country-metrics">
            <div className="metric"><div className="num">{labelInfo.offices}</div><div className="label">Offices</div></div>
            <div className="metric"><div className="num">{labelInfo.employees}</div><div className="label">Employees</div></div>
          </div>
        </div>
      )}
      {debugMode && (
        <div style={{ position: 'absolute', top: '10px', right: '10px', background: 'rgba(0, 0, 0, 0.85)', color: '#0f0', padding: '16px', borderRadius: '8px', fontFamily: 'monospace', fontSize: '12px', zIndex: 1000, minWidth: '300px', border: '1px solid #0f0' }}>
          <div style={{ marginBottom: '12px', fontSize: '14px', fontWeight: 'bold', color: '#fff' }}>🎯 DEBUG MODE (Press D to toggle)</div>
          <div style={{ marginBottom: '8px' }}><strong>Current POV:</strong></div>
          <div style={{ marginBottom: '4px' }}>lat: {currentPOV.lat.toFixed(4)}</div>
          ...
          <div style={{ marginBottom: '12px', paddingTop: '12px', borderTop: '1px solid #0f0' }}>
            ...
            <input 
              ...
              max="4.0" 
              step="0.1" 
              value={altitudeOverride}a// src/App.tsx
import React, { useEffect, useMemo, useRef, useState } from 'react'
import Globe from 'react-globe.gl'
import * as THREE from 'three'
import { feature } from 'topojson-client'
    ...
    <div id="globeRoot">
      ...
      {debugMode && (
        <div style={{ position: 'absolute', top: '10px', right: '10px', background: 'rgba(0, 0, 0, 0.85)', color: '#0f0', padding: '16px', borderRadius: '8px', fontFamily: 'monospace', fontSize: '12px', zIndex: 1000, minWidth: '300px', border: '1px solid #0f0' }}>
          ...
          <div style={{ marginTop: '12px', fontSize: '11px', color: '#888', lineHeight: '1.4' }}>Drag to rotate - Click country to select - Adjust view, then copy coords</div>
        </div>
      )}
